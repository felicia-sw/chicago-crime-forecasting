...
% TODO: confirm author order; add ORCIDs (Springer requests ORCID for all authors).
\author[1]{Felicia Sword\thanks{Corresponding author: \texttt{felicia.sword@gmail.com}}}
\author[1]{Christopher Andreas}
\affil[1]{\small Department of [TODO], [Institution], [City], [Country]}
\date{}
...
loss differential averaged across districts per origin week, with Benjamini--Hochberg control of the
false discovery rate across the full family of tests. We interpret the XGBoost member with SHAP
...
\begin{table}[t]\centering
\caption{Diebold--Mariano tests (squared error, HLN correction, Benjamini--Hochberg FDR-corrected),
main split. Negative statistic favours the ensemble; $^\ast$ denotes significance at FDR 5\%.}
\label{tab:dm}
\small
...
regime-adaptive blend improves on the static blend by 7--11\% (RMSE) during the acute shock, but by
approximately 0\% (occasionally slightly negative) in the calm recovery. This improvement is, however,
\emph{not} statistically significant after correcting for multiple comparisons (the strongest case is
the acute-shock window at $h{=}1$: raw $p=0.041$, $p_{\mathrm{BH}}=0.096$); it is a numerical,
regime-contingent edge rather than a significant accuracy gain. This is the honest form of H3:
the optimal mix shifts with regime, but adaptive weighting does not significantly beat a simple static
inverse-RMSE blend even at the structural break. The unambiguous, statistically robust result is that
the ensemble (including the static blend) significantly beats the best single model across the shock.
...
\noindent\textbf{Funding.} TODO: state any funding, or ``The authors received no specific funding for
this study.''\\[2pt]
\textbf{Competing interests.} The authors declare that they have no competing interests.\\[2pt]
\textbf{Ethics approval.} Not applicable. This study uses publicly available, aggregate,
non-identifiable incident counts and involves no human participants.\\[2pt]
\textbf{Data availability.} The raw data are publicly available from the City of Chicago Data Portal
(dataset \texttt{ijzp-q8t2}). The processed weekly panels, all source code, and computed results are
available at: \url{https://github.com/felicia-sw/chicago-crime-forecasting}.\\[2pt]
\textbf{Author contributions.} TODO (e.g., F.S.\ designed and implemented the methodology, ran the
experiments and wrote the manuscript; C.A.\ supervised the work and reviewed the manuscript).
